...
export const addProfile = (modeName, newProfileName) => {
  browser.storage.local.get(['settings'])
  .then(({ settings }) => {
    const standardSettings = settings[modeName].find((p) => p.name === 'default').settings;
...
export const resetProfile = (modeName, profileName) => {
  fetch(`/default_${modeName.toLowerCase()}.json`)
  .then((response) => {
    ...
    .then(({ settings }) => {
      settings[modeName] = settings[modeName].map((profile) => {
        if (profile.name === profileName) {
          const defaultProfileSettings = modeProfiles.profiles.find((profile) => profile.name === profileName);
          defaultProfileSettings.builtIn = true;
...
export const addProfileGroup = (newProfileGroupName) => {
  ...
  .then(({ profileGroups, activeProfileGroup }) => {
    profileGroups.push({
      ...
      settings: profileGroups.find((p) => p.name === 'default').settings
    });
...
export const deleteProfileGroup = (profileGroupName) => {
  ...
  .then(({ profileGroups }) => {
    ...
    browser.storage.local.set({ profileGroups, activeProfileGroup: 'default' })
    .then(() => {
      msg(1, 'storageChange');
    });
  });
